function [crossTest, crossTrain]=apply_zilla_models_SAT_omiss(dataset, preid, seed, mymodel, omiss)
...
keepsolver=setdiff([1:numberSolver], omiss);
for l=1:PBS_option.NumCrossValidation
 ...
 %       outfilename=sprintf('%s-%s-%s-%d', PBS_option.Predictor,PBS_option.ScoreType,PBS_option.Cost,l);
        tmpresult=PBS_loader(RT(:,keepsolver), SF, F, FT, ALLRT(:, keepsolver), ALLSF, ALLF, ALLFT, PBS_option, outfilename,  outdir, preid);
    ...
    testresult{l}.RT=ALLRT(testIdx,keepsolver);
    ...
    trainresult{l}.RT=ALLRT(trainIdx,keepsolver);
end
