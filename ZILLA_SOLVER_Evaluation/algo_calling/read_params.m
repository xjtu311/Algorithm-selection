...
% 'C:\Program Files\MATLAB71\work\tools\tuning_scenarios\saps\saps-params.txt';    
...
for i=1:length(lines)
    ...
    if ~isempty(matches)
        ...
        param_names{numParam} = param_name;
        ...
        lower_bound = param_transform(lower_bound, param_trafo(numParam));
        upper_bound = param_transform(upper_bound, param_trafo(numParam));
%        done later using config_transform:
%        default_value = param_transform(default_value, param_trafo(i));
...
cat = sort(reverse_index(cat));
cont = sort(reverse_index(cont));
