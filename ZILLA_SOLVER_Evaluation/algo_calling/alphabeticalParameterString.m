function [including_names, just_values] = alphabeticalParameterString(func, x)
...
for i=1:func.dim
    if i>1
        including_names = strcat([including_names ',']);
        just_values = strcat([just_values ',']);
    end
...
%         value = num2str(floor(x(i).*10000 + 0.5)/10000);
    ...
    if i == 1
        including_names = strcat([tmp, '=''', value, '''']);
        just_values = strcat([just_values, '''', value, '''']);
    else
        including_names = strcat([including_names, ' ', tmp, '=''', value, '''']);
        just_values = strcat([just_values, '''', value, '''']);
    end
end
%including_names