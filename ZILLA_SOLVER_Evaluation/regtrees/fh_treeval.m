function [id,nodes,idname]=fh_treeval(Tree,X,subtrees)
...
%   $Revision: 1.1 $  $Date: 2010/10/27 18:14:34 $
...
id = fwd(Tree,X,1:nr,1,zeros(nr,ntrees),subtrees,prunelist,ntrees);

%------------------------------------------------
function id = fwd(Tree,X,rows,thisnode,nodes,subtrees,prunelist,endcol)
...
if endcol > 0
   ...
   if ~isempty(subrows)
      id(subrows) = fwd(Tree,X,subrows,kids(1),nodes,subtrees,prunelist,endcol);
   end
   
   subrows = rows(isright & ~ismissing); % right child node
   if ~isempty(subrows)
      id(subrows) = fwd(Tree,X,subrows,kids(2),nodes,subtrees,prunelist,endcol);
   end

   subrows = rows(ismissing);            % missing, treat as leaf
   if ~isempty(subrows)
%      id(subrows) = Tree.class(thisnode);
      id(subrows) = Tree.proportionleft(thisnode)     * fwd(Tree,X,subrows,kids(1),nodes,subtrees,prunelist,endcol) ... 
                  + (1-Tree.proportionleft(thisnode)) * fwd(Tree,X,subrows,kids(2),nodes,subtrees,prunelist,endcol);
   end
