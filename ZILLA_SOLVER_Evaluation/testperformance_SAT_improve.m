...
function alltestperformance=testperformance_SAT(dataset, seed, mymodel, mode)
if isdeployed
...
allinfo=[];
for preid=1:144
    outputfilename=sprintf('./scripts/Train-%s-%s-%d-%d.mat', dataset, mymodel, preid, seed);
...
for i=1:10
    [allmodeltest, allmodeldata]=apply_zilla_models_SAT(dataset, preidlist(i), seed, mymodel);
    ...
    if strcmp(dataset, 'SATINDU')
      alltestperformance.backupsolver{i}=11;
      backupsolver=11;
    else
...
if mode==2
outputfilename=sprintf('Test1-%s-%s-%d', dataset, mymodel, seed);
end
