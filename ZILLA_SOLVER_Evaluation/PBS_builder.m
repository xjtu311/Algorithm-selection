...
%% debugging purpose.
function result=PBS_builder(RT, SF, F, FT, ALLRT, ALLSF, ALLF, ALLFT, PBS_option, filename, outputdir, preid)
...
PBS=result;
%% let's first do solver subset selection using cross validation
if strcmp(result.Predictor, 'LR') || strcmp(result.Predictor, 'RF')
    PBS_reg_model
else if strcmp(result.Predictor, 'SVM') || strcmp(result.Predictor, 'DT') || strcmp(result.Predictor, 'DF')
        PBS_class_model
    else
        fprintf('The predictor type %s is not supported by this code!', result.Preditor);
        exit
    end
end
