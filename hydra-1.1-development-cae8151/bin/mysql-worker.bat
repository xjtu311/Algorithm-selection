...
@rem ##########################################################################
@rem
@rem  mysql-worker startup script for Windows
@rem
@rem ##########################################################################
...
@rem Add default JVM options here. You can also use JAVA_OPTS and MYSQL_WORKER_OPTS to pass JVM options to this script.
...
@rem Execute mysql-worker
"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %MYSQL_WORKER_OPTS%  -classpath "%CLASSPATH%" ca.ubc.cs.beta.mysqldbtae.worker.MySQLTAEWorker %CMD_LINE_ARGS%
...
:fail
rem Set variable MYSQL_WORKER_EXIT_CONSOLE if you need the _script_ return code instead of
rem the _cmd.exe /c_ return code!
if  not "" == "%MYSQL_WORKER_EXIT_CONSOLE%" exit 1
exit /b 1
