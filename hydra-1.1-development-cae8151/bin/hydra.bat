...
@rem Add default JVM options here. You can also use JAVA_OPTS and HYDRA_OPTS to pass JVM options to this script.
set DEFAULT_JVM_OPTS="-Xmx8192m" "-Dsun.zip.disableMemoryMapping"
...
@rem Execute hydra
"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %HYDRA_OPTS%  -classpath "%CLASSPATH%" ca.ubc.cs.beta.hydra.executors.HydraExecutor %CMD_LINE_ARGS%
...
:fail
rem Set variable HYDRA_EXIT_CONSOLE if you need the _script_ return code instead of
rem the _cmd.exe /c_ return code!
if  not "" == "%HYDRA_EXIT_CONSOLE%" exit 1
exit /b 1
